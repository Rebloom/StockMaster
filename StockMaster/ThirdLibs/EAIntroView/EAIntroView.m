...
- (UIView *)viewForPage:(EAIntroPage *)page atXIndex:(CGFloat *)xIndex {
    ...
    *xIndex += self.scrollView.frame.size.width;
    ...
    if (*xIndex == 960)
    {
        UIButton * btn = [[UIButton alloc] initWithFrame:CGRectMake(40, 385, 240, 50)];
        [btn addTarget:self action:@selector(skipIntroduction) forControlEvents:UIControlEventTouchUpInside];
    ...
    if(page.titleImage) {
        UIImageView * titleImageView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, 220, 220)];
        titleImageView.image = page.titleImage;
...
- (void)buildFooterView {
    ...
    self.skipButton = [[UIButton alloc] initWithFrame:CGRectMake(self.scrollView.frame.size.width/2 - 40, self.pageControl.frame.origin.y-20, 80, self.pageControl.frame.size.height)];
...
- (void)showPanelAtPageControl {
    ...
    [self.scrollView setContentOffset:CGPointMake(self.currentPageIndex * screenWidth, 0) animated:YES];
}
